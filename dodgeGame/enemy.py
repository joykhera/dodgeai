...
class Enemy:
    ...
    def random_position_perimeter(self):
        side = random.randint(1, 4)
        if side == 1:  # top
            self.x = random.randint(0, self.window_width)
            self.y = 0
            self.direction = random.randint(5, 175)
        elif side == 2:  # right
            self.x = self.window_width
            self.y = random.randint(0, self.window_height)
            self.direction = random.randint(95, 265)
        elif side == 3:  # bottom
            self.x = random.randint(0, self.window_width)
            self.y = self.window_height
            self.direction = random.randint(185, 355)
        else:  # left
            self.x = 0
            self.y = random.randint(0, self.window_height)
            self.direction = [random.randint(5, 75), random.randint(275, 355)][random.randint(0, 1)]
