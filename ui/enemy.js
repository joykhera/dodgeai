...
var Enemy = /** @class */ (function () {
    function Enemy(window_width, window_height, color, max_speed, max_radius, enemy_movement, //'aimed' | 'aimed_bounce' | 'random' = 'aimed',
    randomize_radius, randomize_speed, normalize) {
        if (color === void 0) { color = 'rgb(255, 0, 0)'; }
        if (max_speed === void 0) { max_speed = 5; }
        if (max_radius === void 0) { max_radius = 50; }
        if (randomize_radius === void 0) { randomize_radius = false; }
        if (randomize_speed === void 0) { randomize_speed = false; }
    ...
    Enemy.prototype.draw = function (ctx) {
        var x = this.normalize ? this.x * this.window_width : this.x;
        var y = this.normalize ? this.y * this.window_height : this.y;
        var r = this.normalize ? this.radius * this.window_width : this.radius;
        ctx.beginPath();
        ctx.arc(x, y, r, 0, Math.PI * 2, true);
        ctx.fillStyle = this.color;
        ctx.fill();
    };
    Enemy.prototype.reset = function (playerCoords, normalize) {
        if (playerCoords === void 0) { playerCoords = null; }
        ...
        if (this.enemy_movement === 'aimed' ||
            ...
            if (playerCoords !== null) {
                var dist_to_target = Math.sqrt(Math.pow(this.x - playerCoords.x, 2) +
    ...
    Enemy.prototype.move = function (playerCoords) {
        if (this.enemy_movement === 'aimed' || this.enemy_movement === 'aimed_bounce') {
            var new_x = this.x + this.dx;
            var new_y = this.y + this.dy;
            if (new_x < 0 || new_y < 0 || new_x > this.game_width || new_y > this.game_height) {
                ...
                if (this.enemy_movement === 'aimed') {
                    ...
                    var dist_to_target = Math.sqrt(Math.pow((this.x - playerCoords.x), 2) + Math.pow((this.y - playerCoords.y), 2));
                    var randomness = this.speed * 0.1;
    ...
    Enemy.prototype.getEnemyState = function (player, direction, radius, speed) {
        if (direction === void 0) { direction = true; }
        if (radius === void 0) { radius = false; }
        if (speed === void 0) { speed = false; }
        var Enemystate = player ? { x: player.x - this.x, y: player.y - this.y } : { x: this.x, y: this.y };
        if (direction) {
